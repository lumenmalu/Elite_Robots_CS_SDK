...
public:
     ...
     */
    ELITE_EXPORT explicit RtsiIOInterface(const std::string& output_recipe_file, const std::string& input_recipe_file, double frequency);
    ...
    /**
     * @brief Used to input external force sensor data into the system. This data takes effect when ft_rtsi_input_enable is set to true.
     * 
    ...
    /**
     * @return uint32_t Robot safety bits. Bits 0-10: is normal mode | is reduced mode | is protective stopped | is recovery mode | is safeguard stopped | 
     * is system emergency stopped | is robot emergency stopped | is emergency stopped | is violation | is fault | is stoppeddue to safety
     */
    ELITE_EXPORT uint32_t getSafetyStatusBits();

    /**
     * @return uint32_t Analog IO type. Bits 0-3: analog input 0 (bit 0), analog input 1 (bit 1), analog output 0 (bit 2), analog output 1 (bit 3). 0:  current mode; 1: voltage mode
     */
